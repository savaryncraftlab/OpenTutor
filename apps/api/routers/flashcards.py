...
from libs.exceptions import NotFoundError, reraise_as_app_error
...
@router.post(
    ...
    description="Generate flashcards from course content using LLM and FSRS scheduling.",
)
...
@router.post(
    ...
    description="Submit a flashcard review rating and get updated FSRS scheduling.",
)
async def review_flashcard_endpoint(
    ...
    if concept_names and course_id:
        try:
            ...
            logger.warning(
                "Flashcard → LOOM mastery sync failed: services.loom_mastery not found"
            )
...
@router.post(
    ...
    description="Persist a batch of generated flashcards as a reusable asset.",
)
async def save_generated_flashcards(
    ...
    try:
        ...
        raise NotFoundError(
            resource="generated_asset", resource_id=str(body.replace_batch_id)
        ) from exc
...
@router.get(
    "/generated/{course_id}",
    summary="List saved flashcard batches",
    description="Return all saved flashcard batches for a course.",
)
...
@router.get(
    "/lector-order/{course_id}",
    summary="Get LECTOR-ordered flashcards",
    description="Return due flashcards ordered by LECTOR semantic priority instead of FSRS due date.",
)
async def get_lector_ordered_flashcards(
    ...
    use_structured = get_experiment_config(
        user.id,
        "lector_scheduling_v1",
        "use_structured_session",
        default=False,
    )
    ...
    concept_priority = {
        item.concept_name.lower(): (i, item) for i, item in enumerate(review_items)
    }
    ...
    for batch in batches:
        ...
        for idx, card in enumerate(cards):
            ...
            for concept_name, (rank, item) in concept_priority.items():
                if (
                    concept_name in front
                    or concept_name in concept
                    or concept in concept_name
                ):
                    priority_score = rank
                    reason = item.reason
                    break
...
@router.get(
    ...
    description="Return flashcards due for review today across all saved batches.",
)
...
@router.get(
    "/confusion-pairs/{course_id}",
    summary="Get confusion pairs",
    description="Return confused concept pairs for side-by-side comparison review.",
)
async def get_confusion_pairs(
    ...
    if pairs:
        ...
        for pair in pairs:
            ...
            pair["description_a"] = (
                node_a.description if node_a and node_a.description else None
            )
            pair["description_b"] = (
                node_b.description if node_b and node_b.description else None
            )
