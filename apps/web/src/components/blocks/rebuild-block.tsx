...
export function RebuildBlock({
  ...
  return (
    <div
      role="form"
      aria-label="Rebuild block"
      className={`flex flex-col gap-3 p-4 ${className ?? ""}`.trim()}
      data-testid="rebuild-block"
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          Fill the missing code gaps
        </span>
      </div>

      <div
        id={`rebuild-block-prompt-${problemId}`}
        data-testid="rebuild-block-prompt"
      ...
      <p className="text-xs text-muted-foreground">
        Replace every <code># TODO</code> with the correct code. Whitespace is
        normalized.
      </p>

      <div
        data-monaco-editor-root
        data-testid="rebuild-block-editor"
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          data-testid="rebuild-block-submit"
        >
      ...
      {submitError ? (
        <p
          ...
          data-testid="rebuild-block-submit-error"
        >
          {submitError}
        </p>
      ) : null}
